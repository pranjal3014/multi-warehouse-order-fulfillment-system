...
public class UserMapper {
	...
	public UserResponse toResponse(User user) {
		...
		return UserResponse.builder().userId(user.getUserId()).userName(user.getUserName())
				.userEmail(user.getUserEmail()).userContact(user.getUserContact()).role(user.getRole())
				.userStatus(user.getUserStatus()).warehouseId(user.getWarehouseId()).createdAt(user.getCreatedAt())
